...
class CreateOrderSpec extends ApiSpec {
  it should behave like correctShopifyRequestBehaviour(
    apiRequest = CreateOrder(
      Order(
        ...
        totalLineItemsPrice = None,
        totalLineItemsPriceSet = None,
        subtotalPrice = None,
        totalPrice = None,
        totalPriceSet = None,
    ...
    expectedUrl = s"http://localhost:$port/admin/api/2022-01/orders.json",
    expectedMethod = Method.POST,
    expectedRequestBody = Some(
      """{"order":{"line_items":[{"variant_id":447654529,"quantity":1,"price":"1.0"}]}}"""
    ),
    mapping = post("/admin/api/2022-01/orders.json")
